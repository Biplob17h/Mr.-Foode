import React, { useContext, useEffect, useState } from 'react';
import { Link } from 'react-router-dom';
import img from '../../../assets/png-transparent-mr-krabs.png'
import countTotalPrice from '../../../Components/CountsTotalPrice';
import { AuthContext } from '../../../ContextApi/UserContext';

const Header = () => {
    const { user } = useContext(AuthContext)
    const [orders, setOrders] = useState([])
    useEffect(()=>{
        fetch(`https://mr-foode-server.vercel.app/order?email=${user?.email}`)
        .then(res => res.json())
        .then(data => setOrders(data))
    },[user?.email, orders])
     
    const totalPrice = countTotalPrice(orders)
    return (
        <div className="navbar  rounded-md">
            <div className="flex-1">
                <img className='w-20' src={img} alt="" />
                <h1 className="normal-case font-bold ctext1 text-2xl lg:text-5xl">Mr. Foode</h1>
            </div>
            <div className="flex-none">
                <Link to='/'><button className='btn btn-ghost'>Home</button></Link>
                <Link to='/order'><button className='btn btn-ghost'>Order</button></Link>
                <Link to='/chef'><button className='btn btn-ghost'>Chef</button></Link>

                {
                    user?.uid
                        ? <>
                            <div className="dropdown dropdown-end">
                                <label tabIndex={0} className="btn btn-ghost btn-circle">
                                    <div className="indicator">
                                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" /></svg>
                                        <span className="badge badge-sm indicator-item">{orders.length}</span>
                                    </div>
                                </label>
                                <div tabIndex={0} className="mt-3 card card-compact dropdown-content w-52 bg-base-100 shadow">
                                    <div className="card-body">
                                        <span className="font-bold text-lg">{orders.length} Items</span>
                                        <span className="text-info">Subtotal: {totalPrice}$</span>
                                        <div className="card-actions">
                                            <Link to='/settings/cart'><button className="btn btn-primary btn-block">View cart</button></Link>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div className="dropdown dropdown-end ml-5">
                                <label tabIndex={0} className="btn btn-ghost btn-circle avatar">
                                    <div className="w-10 rounded-full">
                                        <img src="/images/stock/photo-1534528741775-53994a69daeb.jpg" alt='' />
                                    </div>
                                </label>
                                <ul tabIndex={0} className="menu menu-compact dropdown-content mt-3 p-2 shadow bg-base-100 rounded-box w-52">
                                    <li>
                                        <Link to='/settings/profile' className="justify-between">
                                            Profile
                                        </Link>
                                    </li>
                                    <li><Link to='/blogs'>Blogs</Link></li>
                                    <li><Link to='/aboutus'>About us</Link></li>
                                    <li><Link to='/notfound'>Settings</Link></li>
                                </ul>
                            </div>
                        </>
                        : <>
                            <Link to='/login'><button className='btn btn-ghost'>Login</button></Link>
                        </>
                }
            </div>
        </div>
    );
};

export default Header;